...
const handler = async (req, res) => {
  if (req.method === "POST") {
    const { image, parts, product, screens, shifts } = req.body;

    try {
      let sql = `
          INSERT INTO product_rel SET product_id= ?, subproduct_id=?, screen_id= ?, productline_id= ?, instruction_id=?, shift_id=?
        `;

      const result = await query({
        query: sql,
        values: [product, parts, screens, 1, image, shifts],
      });
